...
def triangulate_inv(mtx1, mtx2):
    ...
    p3ds_2 = []
    for uv1, uv2 in zip(uvs1, uvs2):
        uv1 = np.concatenate((uv1, [1]), axis=None)
        uv2 = np.concatenate((uv2, [1]), axis=None)
        _p3d_1 = np.matmul(uv1, np.linalg.inv(mtx1))
        p3ds_1.append(_p3d_1)
        _p3d_2 = np.matmul(uv2, np.linalg.inv(mtx2))
        p3ds_2.append(_p3d_2)
    p3ds_1 = np.array(p3ds_1)
    p3ds_2 = np.array(p3ds_2)

    print("triangulated points 1:", p3ds_1)
    print("triangulated points 2:", p3ds_2)
    ...
    #for _c in connections:
        ...
        #print(p3ds[_c[0]])
        #print(p3ds[_c[1]])
        #ax.plot(xs = [p3ds_2[_c[0],0], p3ds_2[_c[1],0]], ys = [p3ds_2[_c[0],1], p3ds_2[_c[1],1]], zs = [p3ds_2[_c[0],2], p3ds_2[_c[1],2]], c = 'red')
    ax.scatter3D(p3ds_2[:, 0], p3ds_2[:, 1], p3ds_2[:,2])
    ax.set_title('This figure can be rotated.')
    #uncomment to see the triangulated pose. This may cause a crash if youre also using cv.imshow() above.
    plt.show()
